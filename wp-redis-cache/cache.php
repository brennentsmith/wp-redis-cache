...
function refresh_wp_redis_cache( $new, $old, $post )
{
	...
	if($new == "publish")
	{
		...
		$redis = new Predis_Client('redis://raven.internal.wildwingstudios.com');
    ...
    $redis->del("ssl_".$redis_key);
		...
		$redis->del($redis_key);
    $redis->del("ssl_".$redis_key);
        clear_pagespeed($_SERVER['REQUEST_URI']);
	}
}
...
function clear_wp_redis_cache()
{
	...
	echo $i . " of " . $wp_query  -> found_posts . " pages was cleared from cache"; 
	die();
}
...
jQuery(document).ready(function($) {
	jQuery('.clear-ww-cache').click(function(){
		event.preventDefault();
		var data = {
			action: 'clear_wp_redis_cache'
		};
		var ajaxurl = '<?php echo admin_url('admin-ajax.php'); ?>'; // required for purging on normal pages
